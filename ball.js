...
make circles move, bouncing around the screen in gentle arcs
have the circles change color as they move
    prbably gonna want to fid an array of pastel colors.  
    Think I can make the circles shift through the array of colors to gradually change?
    could be kinda cool.
add onclick element that removes the circle
create an exploding effect for the removal
Do I need to add these to an array??
add an onclick event that adds an additional circle to the page
...
function addCircleOnClick(){
    ...
    //clicking on the page should create a new circle and add it to the array?
}
...
function popOnClick(event){
    ...
    let eventX = event.srcElement.x + halfCircle/2;//TO DO! X axix of mouse;
